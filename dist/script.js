...
function () {
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      var body = document.querySelector('body'); //доступ ко всему элементу body
      ...
      var recursy = function recursy(element) {
        ...
        element.childNodes.forEach(function (node) {
          // childNodes - дочерние элементы, которые содержит element и приходят они в виде массива
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, "").length > 0) {
            ...
            if (!node.parentNode.getAttribute('data-fz')) {
              var value = window.getComputedStyle(node.parentNode, null).fontSize; // получаем fontsize parentNode.
              // parentNode - это родитель, в данном случае, текстового узла. А родителем является тег, который его оборачивает (div, span и т.д.)

              node.parentNode.setAttribute('data-fz', +value.replace(/px/g, "")); // создаем кастомный аттрибут, в котором будем хранить данные о масштабе "по-умолчанию", чтобы мы могли вернуться к ниму

              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fz') * _this2.scale + "px"; // Логика такова, мы берем размер шрифта из элемента, умножаем на масштаб (который мы получили при клике на кнопку и прибавяем px, для того чтобы стили сработали)
            } else {
